...
Regressão foi utilizada para analisar se a quantidade de `docentes` tem uma associação significativa com `produção do programa`. Os resultados da regressão indicam que um modelo com o preditore no formato `produção do programa` = -41 + 5*`docentes` explicam 81,46% da variância da variável de resposta (R2 = 0,8146). A quantidade de `docentes` tem uma relação significativa com a `produção do programa` (b = [4.269955; 5.356786], IC com 95%). O aumento de 1 unidade de na variável `docentes` produz uma mudança de 5 unidades na variável `produção do programa`.
